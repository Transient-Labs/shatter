from brownie import ShatterV2, accounts, reverts, chain
import pytest
import base64
import json
...
def contract():
    return ShatterV2.deploy("Test", "TST", accounts[1].address, 500, accounts[2].address, 100, shatter_time, {"from": accounts[0]})

@pytest.fixture(scope="class")
def contract1():
    return ShatterV2.deploy("Test", "TST", accounts[1].address, 500, accounts[2].address, 1, shatter_time, {"from": accounts[0]})
...
class TestShatterByFirstBuyer:
    ...
    def test_shatter(self, contract):
        ...
        assert contract.balanceOf(accounts[5].address) == 100
...
class TestShatterMultipleTransfers:
    ...
    def test_shatter(self, contract):
        ...
        assert contract.balanceOf(accounts[6].address) == 100
...
class TestERC721SingleShatter:
    def test_mint_and_shatter(self, contract1):
        ...
        assert(
            ...
            len(tx1.events) == 1 and
            len(tx2.events) == 3 and # 1 transfer event to accounts[0], 1 transfer to 0 address, and Shatter
            contract1.balanceOf(accounts[0].address) == 1
        )
    ...
    def test_transfer_from_owner(self, contract1):
        ...
        assert(
            contract1.ownerOf(1) == accounts[0].address and
            contract1.balanceOf(accounts[0].address) == 1 and
    ...
    def test_transfer_from_approved_for_all(self, contract1):
        contract1.transferFrom(accounts[0].address, accounts[8].address, 1, {"from": accounts[6]})
        assert(
    ...
    def test_transfer_owner_again(self, contract1):
        contract1.transferFrom(accounts[8].address, accounts[0].address, 1, {"from": accounts[8]})
        assert(
            contract1.ownerOf(1) == accounts[0].address and
            contract1.balanceOf(accounts[0].address) == 1
        )
